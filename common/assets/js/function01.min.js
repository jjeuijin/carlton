$(document).ready(function() {
    $(".enter_submit").keydown(function(event) {
        var tg = $(this);
        if (event.keyCode == 13 && tg.val().length !== 0) {
            $('.submit').trigger('click');
        }
    });
    $(".enter_next").keydown(function(event) {
        var tg = $(this);
        if (event.keyCode == 13 && tg.val().length !== 0) {
            var inputs = tg.closest('form').find('input');
            inputs.each(function(i, v) {
                if ($(v).attr('name') == tg.attr('name')) {
                    inputs[i + 1].focus();
                }
            });
        }
    });
    if ($('textarea').length > 0 && !$('textarea.none_func').length > 0) {
        $.each($('textarea'), function(i, v) {
            textarea_resize($(v), 50);
            $(v).css({
                'resize': 'none',
                'overflow-y': 'hidden'
            });
...
function validation(data, func) {
    ...
    switch (get_cookie('lang')) {
        case 'english':
            ...
            msg3 = 'It does not handle Korean with only consonants and vowels or English';
            ...
            msg9 =
                'can only be a combination of alphanumeric characters';
        ...
        case 'japan':
            ...
            msg8 = '入力されたパスワードとパスワードの確認が一致しません。';
...
function search_zip() {
    new daum.Postcode({
        oncomplete: function(data) {
            ...
            $('.v_ZipCode').val(data.zonecode);
            $('.v_Address').val(fullAddr);
            $('.v_AddressDetails').focus();
        }
    }).open();
}
...
function str_check(st) {
    if (location.href.indexOf(st) != -1) return true;
    return false;
}
...
function limit_length(obj, int) {
    if (obj.value.length > int) {
        obj.value = obj.value.substr(0, obj.value.length - 1);
    }
}

function set_dash(tg) {
    var phone = document.getElementById(tg.getAttribute('id'))
    var pval = phone.value;
    var auto_dash = function(str) {
    ...
    $(phone).val(auto_dash(pval));
}

function numb_check(object) {
    var numb_format = function(num) {
        num = num.replace(/,/g, "")
    ...
    var num, chk, tmp = object.value;
    num = tmp.replace(/,/g, "");
    if (isNaN(num)) {
        var msg = '숫자로만 입력해 주세요.';
        switch (get_cookie('lang')) {
            case 'english':
                msg = 'Please enter only numbers';
                break;
        ...
        if (num.indexOf('.') != -1) {} else {
            object.value = numb_format(num);
        }
    }
}

function check_eve(tg) {
    var tg = $(tg);
    if (tg.is(':checked')) {
        tg.next('input').prop('disabled', true);
    } else {
        tg.next('input').prop('disabled', false);
    }
}

function check_value(tg, op) {
    var tg = $(tg);
    var check, msg;
    var value = tg.val();
    switch (op) {
...
function get_count(val1, val2) {
    return get_period(val1, val2) + 1;
}

function get_all_date(val1, val2) {
    var cnt = get_count(val1, val2);
    var ary = [];
    for (var i = 0; cnt > i; i++) {
        var idx = i;
        ary.push(calculation_date(val1, i, '+'));
        get_week(calculation_date(val1, i, '+'));
    }
    return ary;
}
...
function del_zero(number) {
    return number.length >= 2 && number.substr(0, 1) == 0 ? number.substr(1, number.length - 1) : number;
}

function textarea_resize(ts, bsize) {
    var tg = $(ts);
    var csize = (tg.prop('scrollHeight') >= bsize) ? tg.prop('scrollHeight') + 'px' : bsize + 'px';
    tg.css('height', csize);
}
...
function only_number(ts, min, max) {
    var str = ts.value;
    str = new String(str);
    str = str.replace(/[^0-9]/g, '');
    if (str < parseInt(min) || str > parseInt(max) || str === '00' || str === '000') str = '';
    ts.value = str;
}

function chacked_all(tg, id) {
    function evt(arg) {
        $.each($('.' + id), function(i, v) {
            if ($(v).attr('disabled') != 'disabled') {
                $(v).prop('checked', arg);
                if ($('.dcheck').length > 0) $('.dcheck').prop('disabled', arg);
            }
        });
    }
    if ($(tg).is(':checked')) evt(true);
    else evt(false);
}

function checked_checkbox_all(tg, int) {
    var tg =
        $(tg);
    if (tg.is(':checked')) {
        $('.check' + int).prop('checked', true);
    } else {
        $('.check' + int).prop('checked', false);
    }
}
...
function get_segment(n, v) {
    var seg_array = get_segment_array();
